...
public class SelectBuilder<T> {
    ...
    public <U> SelectBuilder<T> send(ChannelHandle.SendPort<U> sndPort, U value, LazyWrapper<AsyncAction<T>> then) {
        sndMap.put(sndPort, then);
        options.add(SelectOption.send(sndPort, value));
        return this;
    }

    public <U> SelectBuilder<T> send(ChannelHandle.SendPort<U> sndPort, U value, AsyncAction<T> then) {
        return send(sndPort, value, () -> then);
    }

    public <U> SelectBuilder<T> receive(ChannelHandle.ReceivePort<U> rcvPort, Function<U, AsyncAction<T>> then) {
